...
export function DeliverToControl({ className }: { className?: string }) {
  ...
  // region.label / option.hint are message keys (DEMO_DELIVER_REGIONS)
  const label = t(region.label);
  const [tipBefore, tipAfter = ''] = t('demo.deliverTo.tip').split(
    '{region}',
  );
      ...
      <DropdownMenu.Root>
        <DropdownMenu.Trigger asChild>
          <button
            ...
            aria-label={t('demo.deliverTo.ariaLabel', { region: label })}
          >
            <MapPin
      ...
      {tipOpen ? (
        ...
        >
          <p className='font-semibold text-stone-700'>
            {tipBefore}
            <span className='font-extrabold text-stone-900'>{label}</span>
            {tipAfter}
          </p>
