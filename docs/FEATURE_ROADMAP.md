...
## 🟣 السبرنت 5: تجارة كاملة (3-4 أسابيع)

| # | الميزة | التفاصيل |
|---|---|---|
| **6** | **الشحن الحقيقي** | طرق (standard/express) + مناطق + أسعار من API — حذف "Digital delivery" |
| **7** | **دفتر العناوين** | في الحساب + إعادة استخدام في checkout |
| **8** | **إلغاء/إرجاع RMA** | قبل الشحن + حالة `refunded` + ربط Stripe refunds |
| **9** | **الضريبة** | نسبة ثابتة قابلة للضبط من الإعدادات |
| **10** | **مراجعات "مشترٍ موثّق"** | شرط: طلب مدفوع يحتوي المنتج |
| **11** | **إشعارات إيميل HTML** | shipped/delivered/canceled/refunded + قوالب HTML |
| **12** | **تتبع طلب حقيقي** | رقم شحنة + شركة + أحداث من الداشبورد |
| **13** | **النشرة البريدية + تواصل حقيقي** | استبدال `href='#'` و "In a real app…" |

---

## ⚪ السبرنت 6: العربية + نمو (3-4 أسابيع)

| # | الميزة | التفاصيل |
|---|---|---|
| **12** | **i18n + RTL كامل** | `next-intl`، `lang/dir` ديناميكي، Tailwind logical properties، عملة/لوكال قابلان للتبديل |
| **13** | **تعديل الملف الشخصي + كلمة مرور** | من داخل الحساب (الـ hook موجود) |
| **14** | **Recently Viewed + Recommendations حقيقية** | بعد إصلاح C3 (co-purchase أو same-category+brand) |
| **15** | **Bundles/Lookbooks/Gift Finder** | البنية موجودة؛ تحتاج seeds وربط الروابط بـ API IDs |
| **16** | **Dashboard كامل** | مخزون + تنبيهات، طلبات (شحن/تتبع/refund/فاتورة)، عملاء، تحليلات، إعدادات، إجراءات جماعية، CSV، Audit log |

---

## 📋 خطة التنفيذ المقترحة (Sprints)

| Sprint | المحتوى | معيار النجاح (Definition of Done) |
|---|---|---|
| **0 — إنقاذ (يوم–يومان)** | C1–C6 | `node app.js` يقلع؛ `npm run lint` + `typecheck` + `test` خضراء؛ كل شاشات الداشبورد تفتح؛ `/recommendations` و `/qa` تعيد 200 |
| **1 — دفع/مخزون** | P1–P5 + W1,W2,W6 | اختبار: عميلان يدفعان آخر قطعة → واحد يُخدم، الآخر refund/needs_attention؛ لا خصم مزدوج؛ إلغاء مدفوع = refund |
| **2 — أمان/Config** | S1–S9 + وحدة config + error handler + indexes | لا تسريب إيميلات؛ rate limit يعمل خلف proxy؛ `NODE_ENV` مطلوب عند الإقلاع؛ اختبارات أمان تمر |
| **3 — متجر يبيع** | W3–W8 + ميزات 1,2,5 | مسار كامل: بحث → تصنيف → منتج بـ variant → سلة ضيف → دخول (`?redirect=`) → دفع → نجاح بدون فقدان `order_id` |
| **4 — SEO/محتوى** | ميزات 3,4,5,9 | Lighthouse SEO ≥ 90؛ PDP مفهرسة بـ JSON-LD؛ sitemap/robots يعملان |
| **5 — تجارة كاملة** | ميزات 6,7,8,10,11 | شحن/ضريبة/عناوين/إرجاع/إشعارات تعمل end-to-end |
| **6 — عربية/نمو** | ميزة 12 + 13–16 | المتجر RTL بالكامل؛ تتبع/نشرة/Bundles تعمل |
...
| `apps/api/render.yaml` | healthCheckPath + SMTP_* + rootDir | 2 |
| `apps/api/package.json:9` | تشغيل كل الاختبارات | 0 |

---

## 🧹 تنظيف كود ميت (Tech Debt)

| العنصر | الموقع | الإجراء |
|---|---|---|
| `@trendvaulta/api-client` | `packages/api-client/` | **حذف** — غير مستخدم من أي تطبيق |
...
| `deliverRegion` | غير مستخدم | **حذف** |
| `<title>Craftify Admin Dashboard</title>` | `dashboard/index.html` | تصحيح → TrendVaulta |
| `packages/ui` | مرجع في README/AGENTS | **إزالة المراجع** — غير موجود |
